...
export async function humanDelay(
  min = testConfig.delays.short.min,
  max = testConfig.delays.short.max,
) {
...
export async function smartScroll(context, selector) {
  try {
    ...
    await humanDelay(
      testConfig.delays.short.min / 2,
      testConfig.delays.short.max / 2,
    );
  }
}
...
export function setupErrorLogging(page) {
  page.on("console", (msg) => {
    ...
    const consoleIgnores = testConfig.logging?.ignoreConsoleErrors || [];
  ...
  page.on("pageerror", (exception) => {
    const pageIgnores = testConfig.logging?.ignorePageErrors || [];
  ...
  page.on("requestfailed", (request) => {
    ...
    if (testConfig.network?.suppressRequestFailedLogs) return;

    // If the URL matches any of the configured ignore patterns, skip logging
    const ignorePatterns = testConfig.network?.ignoreFailedRequests || [];
    ...
    const ignoreFailureErrors =
      testConfig.logging?.ignoreRequestFailureErrors || [];
